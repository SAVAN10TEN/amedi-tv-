...
async function startServer() {
  ...
  // --- First Iraqi Bank (FIB) Payment Gateway Integration ---
  
  interface FibSession {
    paymentId: string;
  ...
  const fibSessions = new Map<string, FibSession>();

  // Cleanup expired sessions every 10 minutes
  setInterval(() => {
    const tenMinsAgo = Date.now() - 10 * 60 * 1000;
    for (const [id, value] of fibSessions.entries()) {
      if (value.createdAt < tenMinsAgo && value.status === "UNPAID") {
        fibSessions.delete(id);
      }
    }
  }, 10 * 60 * 1000);

  // 1. Create a payment request with FIB
  app.post("/api/payments/fib/create", async (req, res) => {
    ...
    const amount = amountMap[period as '1month' | '6months' | '1year'];
    const paymentId = "fib-pay-" + Math.random().toString(36).substring(2, 11) + "-" + Date.now().toString(36);
    const periodLabel = period === '1month' ? '1 Month' : period === '6months' ? '6 Months' : '1 Year';
    const randomizedCode = "AMEDI-" + period.replace('s', '').toUpperCase() + "-" + Math.floor(100000 + Math.random() * 900000);

    const isReal = !!(process.env.FIB_CLIENT_ID && process.env.FIB_CLIENT_SECRET);

    if (isReal) {
      try {
        const tokenUrl = process.env.FIB_TOKEN_URL || "https://api.fib.iq/auth/realms/fib/protocol/openid-connect/token";
        const paymentUrl = process.env.FIB_PAYMENTS_URL || "https://api.fib.iq/payments";

        const authHeader = Buffer.from(`${process.env.FIB_CLIENT_ID}:${process.env.FIB_CLIENT_SECRET}`).toString('base64');
        ...
        if (!authRes.ok) {
          ...
          throw new Error(`FIB auth failed: ${authRes.statusText} - ${errorText}`);
        }

        const authData = await authRes.json() as any;
        const accessToken = authData.access_token;

        const checkoutRes = await fetch(paymentUrl, {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            amount,
            currency: "IQD",
            description: `Amedi TV Premium Subscription - ${periodLabel}`,
            callbackUrl: `${req.protocol}://${req.get('host')}/api/payments/fib/webhook`
          })
        });

        if (!checkoutRes.ok) {
          const errorText = await checkoutRes.text();
          throw new Error(`FIB create payment failed: ${checkoutRes.statusText} - ${errorText}`);
        }

        const paymentData = await checkoutRes.json() as any;
        
        fibSessions.set(paymentData.paymentId || paymentId, {
          paymentId: paymentData.paymentId || paymentId,
          status: "UNPAID",
          amount,
          period: period as any,
          createdAt: Date.now(),
          activationCode: randomizedCode,
        ...
        return res.json({
          ...
          personalPaymentToken: paymentData.personalPaymentToken,
          amount,
          period,
          isSimulated: false
        });
      } catch (err: any) {
        console.error("[FIB API Error] Real integration failed. Dropping back to robust simulated gateway sandbox:", err.message || err);
      }
    }

    // Dynamic sandbox simulated fallback payment
    const simQrCode = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=fib-payment://${paymentId}?amount=${amount}%26currency=IQD`;

    fibSessions.set(paymentId, {
      paymentId,
      status: "UNPAID",
      amount,
      period: period as any,
    ...
    return res.json({
      ...
      paymentId,
      qrCode: simQrCode,
      readableCode: Math.floor(100000 + Math.random() * 900000).toString(),
      amount,
      period,
      isSimulated: true
    });
  ...
  // 2. Poll/query status of a payment (real or simulated)
  app.get("/api/payments/fib/status/:paymentId", async (req, res) => {
    const { paymentId } = req.params;
    const session = fibSessions.get(paymentId);
    if (!session) {
      return res.status(404).json({ success: false, message: "Payment session not found" });
    }

    const isReal = !!(process.env.FIB_CLIENT_ID && process.env.FIB_CLIENT_SECRET);
    if (isReal && session.status !== "PAID") {
      try {
        const tokenUrl = process.env.FIB_TOKEN_URL || "https://api.fib.iq/auth/realms/fib/protocol/openid-connect/token";
        const statusUrl = `https://api.fib.iq/payments/${paymentId}/status`;
        ...
        if (authRes.ok) {
          ...
          if (checkRes.ok) {
            ...
            if (checkData.status === "PAID") {
              ...
              const codeToRegister = session.activationCode.trim().toUpperCase();
              if (!activationConfig.validCodes.includes(codeToRegister)) {
                activationConfig.validCodes.push(codeToRegister);
                try {
                  fs.writeFileSync(activationConfigPath, JSON.stringify(activationConfig, null, 2), "utf-8");
                  console.log(`[FIB Real Payment] Dynamically registered valid activation code to database: ${codeToRegister}`);
                  broadcastEvent("activation-config-updated", { config: activationConfig });
                } catch (err) {
                  console.error("Failed to persist newly verified real code:", err);
                }
              }
            } else if (["DECLINED", "CANCELLED", "EXPIRED"].includes(checkData.status)) {
    ...
    res.json({
      ...
      activationCode: session.status === "PAID" ? session.activationCode : undefined
    });
  });

  // 3. Webhook endpoint for live payment notifications directly from FIB server
  app.post("/api/payments/fib/webhook", (req, res) => {
    const { paymentId, status } = req.body;
    console.log(`[FIB Webhook] Received status notification. PaymentId: ${paymentId}, Status: ${status}`);
    ...
    if (paymentId && status === "PAID") {
      ...
      if (session && session.status !== "PAID") {
        ...
        if (!activationConfig.validCodes.includes(codeToRegister)) {
          ...
          try {
            fs.writeFileSync(activationConfigPath, JSON.stringify(activationConfig, null, 2), "utf-8");
            console.log(`[FIB Webhook] Successfully registered valid activation code: ${codeToRegister}`);
            broadcastEvent("activation-config-updated", { config: activationConfig });
          } catch (err) {
            console.error("Failed to persist code from webhook:", err);
          }
        }
      }
    }
  ...
  // 4. Manual developer simulation endpoint for local testing of checkout flow in sandbox
  app.post("/api/payments/fib/simulate-complete", (req, res) => {
    ...
    const session = fibSessions.get(paymentId);
    if (!session) {
      return res.status(404).json({ success: false, message: "FIB session not found" });
    }
    ...
    if (session.status !== "PAID") {
      ...
      if (!activationConfig.validCodes.includes(codeToRegister)) {
        ...
        try {
          fs.writeFileSync(activationConfigPath, JSON.stringify(activationConfig, null, 2), "utf-8");
          console.log(`[FIB Simulation] Successfully generated & registered activation code: ${codeToRegister}`);
          broadcastEvent("activation-config-updated", { config: activationConfig });
        } catch (err) {
          console.error("Failed to persist simulated activation code:", err);
        }
      }
    }

    res.json({ success: true, message: "Payment status simulated successfully to PAID", session });
  });
